...
from rag_model import main_rag_process
...
def show_explore_documents(publications):
    ...
    for pub in publications:
        if pub.get("image_link") or pub.get("pdf_link"):
...
def show_pdf_qna_page():
    ...
    if selected_pdf:
        ...
        if st.button("Get Answer"):
            if user_query:
                answer = main_rag_process(pdf_url, user_query)  # Use the generated PDF link and user query
    ...
    if st.button("Back to Main"):
        st.session_state.pop('selected_pdf', None)  # Clear selected PDF
        st.session_state.pop('pdf_url', None)  # Clear PDF URL
        st.rerun()
